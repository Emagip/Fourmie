...
def combien_de_simu():
    ...
    simu_label.grid(row=4,
                    column=0)
    simu_entry = Entry(simu.Fond,
                       textvariable=simu_var)
    simu_entry.grid(row=5,
                    column=0)
...
def vitesseFourmi():
    ...
    vitesse_label = Label(simu.Fond,
                          text="À qu'elle vitesse voulez vous que votre fourmi ce déplace")
    vitesse_label.grid(row=0,
                       column=0)
    vitesse_label2 = Label(simu.Fond,
                           text=" 0 : 0.1s (normal), 1 : 0.01s (rapide), 2 : 0.001 (trés rapide), 3 : 0.0001 (supersonique)")
    vitesse_label2.grid(row=1,
                        column=0)
    vitesse_entry = Entry(simu.Fond,
                          textvariable=vitesse_var)
    vitesse_entry.grid(row=2,
                       column=0)
...
def detruire_btn_vitesse():
    ...
    JeuAffichage.config(text='Lancer une simulation avec interface graphique',
                        command = MiseEnPlaceTK)
...
def detruire_btn_simu():
    ...
    simu_entry.destroy()
    simu_label.destroy()
    JeuGraphique.config(text='Lancer plusieurs simulations et obtenir des statistics',
                        command=simuSansTK)
...
def simuSansTK():
    ...
    global simuls
    combien_de_simu()
    JeuGraphique.config(text='valider',
                        command=stats)
...
def MiseEnPlaceTK():
    ...
    vitesseFourmi()
    JeuAffichage.config(text='valider',
                        command=simuTK)
...
JeuAffichage = Button(simu.Fond,
                      ...
                      command=MiseEnPlaceTK)
JeuAffichage.grid(row=3,
                  column=0)
JeuGraphique = Button(simu.Fond,
                      text='Lancer plusieurs simulations et obtenir des statistics',
                      command=simuSansTK)
JeuGraphique.grid(row=6,
                  column=0)
Quitter = Button(simu.Fond,
                 text='Quitter le jeu',
                 command=simu.interface.destroy)
Quitter.grid(row=7,
             column=0)
